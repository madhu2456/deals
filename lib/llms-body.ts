...
/**
 * Shared body for /llms.txt and /llms-full.txt (F219). llms-full.txt mirrors
 * llms.txt exactly — the inventory is already the full public surface, so the
 * "full" variant is the same content. The body is deterministic: no
 * per-request "Generated" timestamp (same stability principle as the sitemap
 * F236 — content must not churn between fetches).
 */
export async function buildLlmsBody(): Promise<string> {
